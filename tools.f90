...
integer :: i,j,k
real(mytype) :: phimax,phimin,cfl,phimax_rezone,phimin_rezone
real(mytype) :: phimax1,phimin1,phimax_rezone1,phimin_rezone1
...
do i=1,xsize(1)
   ...
   if (phi(i,j,k).lt.phimin) phimin=phi(i,j,k)
    if (rezone>0 .and. xstart(1)+i-1 .le. rezone) then
        if (phi(i,j,k).gt.phimax_rezone) phimax_rezone=phi(i,j,k)
        if (phi(i,j,k).lt.phimin_rezone) phimin_rezone=phi(i,j,k)
    endif
enddo
enddo
enddo
...
real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: bx1,bxn

integer :: j,i,k,code
real(mytype) :: can,ut_in,ut_out,ut_in1,ut_out1,y,ut_in_av
!we are in Y-pencil
   ...
   do j=ibm_y_max,ny-1   
      if (istret.ne.0) then
         ut_in1=ut_in1+(yp(j+1)-yp(j))*dz*(bx1(1,j+1,k)-0.5*(bx1(1,j+1,k)-bx1(1,j,k)))
	 ut_out1=ut_out1+(yp(j+1)-yp(j))*dz*(bxn(1,j+1,k)-0.5*(bxn(1,j+1,k)-bxn(1,j,k)))
      endif
	!if (print_flag==1 .and. nrank==0) print *,'bx1',bx1(1,j+1,k),'bxn',bxn(1,j+1,k)
      if (istret.eq.0) then
	 ut_in1=ut_in1+(yly/(ny-1))*dz*(bx1(1,j+1,k)-0.5*(bx1(1,j+1,k)-bx1(1,j,k)))
	 ut_out1=ut_out1+(yly/(ny-1))*dz*(bxn(1,j+1,k)-0.5*(bxn(1,j+1,k)-bxn(1,j,k)))
      endif
   enddo
   enddo
   ...
   can=(ut_out-ut_in-ujicf)/(zlz*(yly-yp(ibm_y_max)))   
   do k=1,xsize(3)
   do j=ibm_y_max,ny
      bxn(1,j,k)=bxn(1,j,k)-can
   enddo
   enddo
...
real(mytype),dimension(ysize(1),ysize(2),ysize(3)) :: bx1,ux

integer :: j,i,k,code
real(mytype) :: can,ut_all1,ut_in1,ut_all,ut_in
...
do j=ibm_y_max,ny-1
  ...
  if (istret.ne.0) ut_in1=ut_in1+(yp(j+1)-yp(j))*(bx1(1,j+1,k)-0.5*(bx1(1,j+1,k)-bx1(1,j,k)))
  if (istret.eq.0) ut_in1=ut_in1+(yly/(ny-1))*(bx1(1,j+1,k)-0.5*(bx1(1,j+1,k)-bx1(1,j,k)))

!do i=1,ysize(1)
!if (istret.ne.0) ut_all1=ut_all1+(yp(j+1)-yp(j))*(ux(i,j+1,k)-0.5*(ux(i,j+1,k)-ux(i,j,k)))
!if (istret.eq.0) ut_all1=ut_all1+(yly/(ny-1))*(ux(i,j+1,k)-0.5*(ux(i,j+1,k)-ux(i,j,k)))
!enddo
...
call MPI_ALLREDUCE(ut_in1,ut_in,1,real_type,MPI_SUM,MPI_COMM_WORLD,code)
ut_in=ut_in/(nz*(yly-yp(ibm_y_max)))
!call MPI_ALLREDUCE(ut_all1,ut_all,1,real_type,MPI_SUM,MPI_COMM_WORLD,code)
!ut_all=ut_all/yly/nz/nx

!can=-2.*xnu*gdt(itr) ! Poisseuille
can=-(1.-ut_in) ! constant flow rate

!if (print_flag==1 .and. nrank==0) print *, nrank, 'UT_in', ut_in, 'error', can

if (ystart(1).eq.1) then
do k=1,ysize(3)
do j=ibm_y_max+1,ny-1
  bx1(1,j,k)=-can+bx1(1,j,k)
enddo
enddo
endif
