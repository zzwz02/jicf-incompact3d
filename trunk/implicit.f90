...
integer :: njicf1,tempa,tempb
real(mytype) :: ujicf1,r2,temp1,temp
...
!check the hole !we are in the Y-pencil
    ...
    if (r2 .le. rjicf) then
        ...
        if (ilit==0) then
            tb2(i,1,k)=min(r2,(itime-ifirst)*r2/10000)
        else
            tb2(i,1,k)=r2
        endif
        temp1=max(tb2(i,1,k),temp1)
        ujicf1=ujicf1+tb2(i,1,k)*dx*dz
        njicf1=njicf1+1
        tb2(i,ysize(2),k)=0.
    endif
enddo
...
!if ((ncly.eq.1).and.(.not.present(npaire))) then
!   if (nrank.eq.0) print *,'Missing parameter npaire when ncly=1, abort computation'
!   call decomp_2d_finalize
!   CALL MPI_FINALIZE(code)
!   call exit(0)
!endif
...
if (ncly==2) then
   ...
   ta2(:,ysize(2),:)=g_n
    ...
    if ((itype.eq.5) .and. (v_jicf.ne.0)) then
        ! determine the processor grid in use
        !call MPI_CART_GET(DECOMP_2D_COMM_CART_X, 2, &
        !    dims, dummy_periods, dummy_coords, code)
    do k=1,ysize(3)
