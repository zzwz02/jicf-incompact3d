...
!if ((itime.lt.10000).and.(ilit.eq.0)) then
!   if (print_flag==1 .and. nrank==0) print *,'ATTENTION WARNING ACHTUNG : rotation source term',itime
!   tg1(:,:,:)=tg1(:,:,:)-1.5*2./9.*uy1(:,:,:)
!   th1(:,:,:)=th1(:,:,:)-1.5*2./9.*ux1(:,:,:)
!endif
...
integer :: ijk,nvect1,nvect2,nvect3,i,j,k,nxyz,tempa,tempb
...
do ijk=1,nvect1
   ta1(ijk,1,1)=(1.-epsi(ijk,1,1))*(xnu/sc*ta1(ijk,1,1)-tb1(ijk,1,1))
enddo
!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! Kasagi Source term !!!
!!!!!!!!!!!!!!!!!!!!!!!!!!
